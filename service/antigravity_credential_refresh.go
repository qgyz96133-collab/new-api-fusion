...
type AntigravityRefreshResponse struct {
	AccessToken  string `json:"access_token"`
	RefreshToken string `json:"refresh_token"`
	ExpiresIn    int64  `json:"expires_in"`
}

func RefreshAntigravityOAuthToken(ctx context.Context, refreshToken string, proxy string) (*AntigravityRefreshResponse, error) {
	...
	data.Set("client_id", "antigravity-cli")
	...
	if resp.StatusCode != http.StatusOK {
		return nil, fmt.Errorf("antigravity refresh failed: %d %s", resp.StatusCode, string(body))
	}

	var result AntigravityRefreshResponse
...
func RefreshAntigravityChannelCredential(ctx context.Context, channelID int, opts AntigravityCredentialRefreshOptions) (*AntigravityOAuthKey, *model.Channel, error) {
	...
	res, err := RefreshAntigravityOAuthToken(refreshCtx, oauthKey.RefreshToken, ch.GetSetting().Proxy)
	if err != nil {
		return nil, nil, err
	}
